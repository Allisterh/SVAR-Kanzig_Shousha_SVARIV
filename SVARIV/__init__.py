...
def simple_plot(title, est, est2,est3,low_ar, upp_ar, low_plugin, upp_plugin,  
                xticks,ylabel, rot=0, ylim0=-3.5, ylim1=3.5,
                xlabel='Months',hori=21, legend=None,saved='a.png',dpi=500):
    '''
    Input:
    ...
    plt.plot(est, color='blue', zorder=1, linewidth=1)
    plt.plot(est2, color='red', zorder=1, linewidth=1)
    plt.plot(est3, color='darkgreen', zorder=1, linewidth=1)

    plt.fill_between(xticks , low_plugin, upp_plugin, color='b', alpha=.1)
    plt.plot(low_ar, '--', zorder=1, linewidth=1,color='orange')
    if legend == None:
        ax.legend(['SVARIV','Cholesky','VAR', '$CS^{plugin}$','$CS^{AR}$'], fontsize=8).set_zorder(5)
    else:
        ax.legend(legend, fontsize=17).set_zorder(5)
    plt.plot(upp_ar, '--', zorder=1, linewidth=1,color='orange')
    ax.grid()
    plt.axhline(y=0, color='black', linestyle='-',linewidth=2.5)
    plt.xticks(list(range(0,len(xticks)+1,10)), list(range(0,len(xticks)+1,10)), rotation=rot)
    plt.yticks()
    ...
    plt.savefig(saved, dpi=dpi)
    plt.tight_layout(rect=[0,0,.8,1])
    return f


def simple_plot_cum(title, est, est2,est3, low_ar, upp_ar,low_plugin, upp_plugin,
                xticks,ylabel, rot=0, ylim0=-3.5, ylim1=3.5,
                xlabel='Months',hori=21, legend=None,saved='a.png',dpi=500):
    '''
    Input:
